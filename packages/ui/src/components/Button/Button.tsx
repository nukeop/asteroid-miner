...
const buttonVariants = cva(
  'inline-flex cursor-pointer items-center justify-center gap-2 font-mono text-sm tracking-wide uppercase transition-colors disabled:pointer-events-none disabled:opacity-50',
  {
    variants: {
      variant: {
        primary: 'bg-crt-text text-crt-bg hover:bg-crt-bright',
        secondary:
          'border-crt-muted bg-crt-surface text-crt-text hover:bg-crt-muted/30 hover:border-crt-text border',
        ghost: 'text-crt-muted hover:text-crt-text hover:bg-crt-surface',
      },
      size: {
        sm: 'h-7 px-3 text-xs',
        md: 'h-9 px-4 text-sm',
